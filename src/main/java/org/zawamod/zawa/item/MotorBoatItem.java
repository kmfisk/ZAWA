...
public class MotorBoatItem extends Item {
    private static final Predicate<Entity> PLACEMENT_TESTER = EntityPredicates.NOT_SPECTATING.and(Entity::canBeCollidedWith);
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        if (raytraceresult.getType() == RayTraceResult.Type.MISS) {
            ...
            double d0 = 5.0D;
            List<Entity> list = worldIn.getEntitiesInAABBexcluding(playerIn, playerIn.getBoundingBox().expand(vector3d.scale(d0)).grow(1.0D), PLACEMENT_TESTER);
            if (!list.isEmpty()) {
            ...
            if (raytraceresult.getType() == RayTraceResult.Type.BLOCK) {
                BoatEntity boatentity = new MotorBoatEntity(worldIn, raytraceresult.getHitVec().x, raytraceresult.getHitVec().y, raytraceresult.getHitVec().z);
